...
class TaskfileManager:
    ...
    def _get_config_safe(self) -> Dict[str, Any]:
        """安全地获取配置，无论session_state是否可用"""
        try:
            # 先检查st对象是否有session_state属性
            if hasattr(st, 'session_state'):
                # 然后检查session_state中是否有taskfiles_config属性
                if hasattr(st.session_state, 'taskfiles_config'):
                    return st.session_state.taskfiles_config
                elif 'taskfiles_config' in st.session_state:
                    return st.session_state.taskfiles_config
                else:
                    # 如果没有，加载并设置它
                    config = self.load_taskfiles_config()
                    try:
                        ...
                        print(f"更新session_state失败: {str(e)}")
                    return config
            else:
                # 如果streamlit会话不可用，直接加载配置
                return self.load_taskfiles_config()
        except Exception as e:
            print(f"获取配置时出错: {str(e)}")
            return dict(self._default_config)  # 返回默认配置的副本
      
    def _update_session_state(self, config: Dict[str, Any]) -> None:
        """安全地更新session_state中的配置"""
        try:
            # 确保st对象有session_state属性
            if hasattr(st, 'session_state'):
                # 使用字典风格访问，避免属性访问可能的问题
                st.session_state['taskfiles_config'] = config
        except Exception as e:
            print(f"更新session_state失败: {str(e)}")
    ...
    def get_active_taskfile(self) -> Optional[str]:
        ...
        if not active and config.get('taskfiles'):
            ...
            self._update_session_state(config)
    ...
    def add_taskfile(self, taskfile_path: str) -> bool:
        ...
        # 如果已存在，不重复添加
        if taskfile_path in config.get('taskfiles', []):
            return True
        ...
        # 如果是第一个Taskfile，设为活动
        if not config.get('active_taskfile'):
    ...
    def remove_taskfile(self, taskfile_path: str) -> bool:
        ...
        if taskfile_path not in config.get('taskfiles', []):
            return False
        ...
        if config.get('active_taskfile') == taskfile_path:
